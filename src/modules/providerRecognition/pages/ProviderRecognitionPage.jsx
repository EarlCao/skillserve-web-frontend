...
export default function ProviderRecognitionPage() {
  ...
  return <div className="flex flex-col gap-4">
    ...
    <ConfirmDialog open={deleteDisclosure.isOpen} onCancel={deleteDisclosure.close} onConfirm={() => deleteBadge.mutate(deletingBadge?.id, { onSettled: () => { deleteDisclosure.close(); setDeletingBadge(null) } })} loading={deleteBadge.isPending} title="Delete badge?" description={`Delete "${deletingBadge?.name ?? ''}"? Assigned providers will lose this badge.`} confirmText="Delete badge" variant="error" />
  </div>
}
