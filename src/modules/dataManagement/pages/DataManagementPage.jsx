...
// Deleted records are purged automatically after the backend's retention_days,
// unless related data still references them (blocked_by).
function purgeLabel({ purge_at: purgeAt, blocked_by: blockedBy }) {
  if (blockedBy) return <span className="text-xs text-base-content/60">Kept — has {blockedBy}</span>
  if (!purgeAt) return <span className="text-base-content/40">—</span>
  const days = differenceInCalendarDays(new Date(purgeAt), new Date())
  if (days <= 0) return <span className="badge badge-error badge-sm">Today</span>
  return <span className={`badge badge-sm ${days <= 7 ? 'badge-warning' : 'badge-ghost'}`}>In {days} {days === 1 ? 'day' : 'days'}</span>
}
...
export default function DataManagementPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card title="Deleted records" description="Deleted records are permanently removed 30 days after deletion; restore them before then or delete them now. Records that related data still depends on (e.g. a user with bookings) are kept.">
        {deleted.isLoading ? <Skeleton className="h-48 w-full" /> : deleted.isError ? <ErrorState title="Could not load deleted records" message={deleted.error?.message} onRetry={deleted.refetch} /> : records.length === 0 ? <div className="flex flex-col items-center gap-2 py-12 text-center"><Archive className="size-10 text-base-content/30" /><p className="font-medium">No deleted records</p></div> : <><div className="overflow-x-auto"><table className="table"><thead><tr><th>Type</th><th>Record</th><th>Deleted</th><th>Auto-delete</th><th className="text-right">Actions</th></tr></thead><tbody>{records.map((record) => <tr key={`${record.resource_type}-${record.resource_id}`}><td className="capitalize">{record.resource_type.replaceAll('_', ' ')}</td><td>{record.label}</td><td>{record.deleted_at ? new Date(record.deleted_at).toLocaleString() : '—'}</td><td className="whitespace-nowrap">{purgeLabel(record)}</td><td className="flex justify-end gap-2">{canRestoreDeleted && <Button size="sm" variant="outline" onClick={() => restore.mutate({ type: record.resource_type, id: record.resource_id })} loading={restore.isPending}><RotateCcw className="size-4" /> Restore</Button>}{canDelete && <Button size="sm" variant="error" disabled={!record.can_permanently_delete} title={record.blocked_by ? `Remove its related data first (${record.blocked_by}).` : undefined} onClick={() => setConfirm(record)}><Trash2 className="size-4" /> Delete permanently</Button>}</td></tr>)}</tbody></table></div><div className="flex justify-end border-t border-base-200 p-4"><Pagination totalItems={deleted.data?.meta?.pagination?.total ?? 0} perPage={10} pagination={pagination} /></div></>}
      </Card>
