...
export default function DataManagementPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ConfirmDialog open={Boolean(confirm)} onCancel={() => setConfirm(null)} onConfirm={() => { if (!confirm) return; remove.mutate({ type: confirm.resource_type, id: confirm.resource_id }, { onSettled: () => setConfirm(null) }) }} loading={remove.isPending} title="Permanently delete record?" description="This action cannot be undone." confirmText="Delete permanently" variant="error" />
    </div>
  )
}
